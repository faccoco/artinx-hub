...
由于对角速度的计算存在较大的噪声，对角速度拟合参数较为困难。因此，对角速度进行积分，得到角度为一个余弦函数，对该余弦函数的参数进行拟合。在计算出能量机关扇叶的θ后，将观测出来的连续的20帧θ送进Ceres库中，进行最小二乘拟合，出来拟合参数后进行拓展卡尔曼滤波预测击打。

## 存在的问题
1. ceres拟合不准确，虽然采用角度进行参数的最小二乘拟合，但是实际应用过程中仍然会出现拟合的参数不准确，导致预测的结果不准确，跟不上大符的扇叶现象
2. yaw角观测不准确，由于pnp的结果得到的yaw观测误差较大，而yaw的获取可以采用其他方式获取。这里提供一个思路: 根据前后两帧pnp解算出来惯性系下的位置，可以计算出一个惯性系的方向向量，该向量一定位于能量机关所在的平面，将该向量投影到XOZ平面，得到的向量与X轴的夹角即为能量机关的yaw角