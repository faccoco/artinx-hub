...
bool inspect(Inspector& f, UndistortCalibratorSettings& x) {
    return f.object(x).fields(
        ...
        f.field("calibZeroTangentDist", x.calibZeroTangentDist), f.field("calibFixPrincipalPoint", x.calibFixPrincipalPoint),
        f.field("outputFileName", x.outputFileName), f.field("writePoints", x.writePoints),
...
double computeReprojectionErrors(const std::vector<std::vector<cv::Point3f>>& objectPoints, const std::vector<std::vector<cv::Point2f>>& imagePoints,
                                const std::vector<cv::Mat>& rvecs, const std::vector<cv::Mat>& tvecs, const cv::Mat& cameraMatrix, const cv::Mat& distCoeffs,
...
bool runCalibration(std::vector<cv::Mat>& rvecs, std::vector<cv::Mat>& tvecs,
    ...
    if(releaseObject) {
        ...
        CAF_LOG_INFO(newObjPoints[mConfig.boardSize.width * (mConfig.boardSize.height - 1)]);
        CAF_LOG_INFO(newObjPoints.back());
    }
...
bool runCalibrationAndSave() {
    ...
    CAF_LOG_INFO((ok ? "Calibration succeeded" : "Calibration failed"));
    CAF_LOG_INFO("avg re projection error =" + std::to_string(totalAvgErr));
