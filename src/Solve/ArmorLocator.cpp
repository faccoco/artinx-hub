        ...
        return { [](start_atom) { ACTOR_PROTOCOL_CHECK(start_atom); },
                 [&](armor_detect_available_atom, Identifier key) {
                     ...
                     for(const auto& armor : data.armors) {
                         ...
                         cv::Mat rvec, tvec;
                         const auto pnpRes = cv::solvePnP(armor.isLargeArmor ? mObjectPointsLarge : mObjectPointsSmall,
                                                          mImagePoint, cameraInfo.cameraMatrix, cameraInfo.distCoefficients, rvec,
                         ...
                         res.targets.push_back({ armorImgCenter, distance2D(armorImgCenter, imgCenter), pointRefCam,
                                                 armor.robotType, armorType, ArmorMotion::Unsure, rmat });
