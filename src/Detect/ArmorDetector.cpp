...
bool inspect(Inspector &f, ArmorDetectorSettings &x) {
    return f.object(x).fields(f.field("globalScale", x.globalScale),
                              f.field("thresholdForBlue", x.thresholdForBlue).invariant(
                                      [](auto &c) { return c.size() == 3; }),
                              f.field("thresholdForRed", x.thresholdForRed).invariant(
                                      [](auto &c) { return c.size() == 3; }),
                              f.field("maxAreaRatio", x.maxAreaRatio), f.field("maxLightAngle", x.maxLightAngle),
                              f.field("maxLightRectRatio", x.maxLightRectRatio),
                              f.field("maxArmorRectRatio", x.maxArmorRectRatio),
                              f.field("maxArmorAngle", x.maxArmorAngle),
                              f.field("minLightBaseAngle", x.minLightBaseAngle),
                              f.field("maxParallelAngle", x.maxParallelAngle),
...
class ArmorDetector final
        ...
        if (GlobalSettings::get().selfColor == Color::Red) {
            ...
            for (int32_t i = 0; i < src.rows; ++i)
                for (int32_t j = 0; j < src.cols; ++j) {
                    ...
                    result.at<uchar>(i, j) = (!isWhite(b, g, r) && b > minB && g < maxG && r < maxR && b * 4 > g + r)
                                             ? 255 : 0;
                }
        } else {
            ...
            for (int32_t i = 0; i < src.rows; ++i)
                for (int32_t j = 0; j < src.cols; ++j) {
                    const auto &col = src.at<cv::Vec3b>(i, j);
                    const int32_t b = col[0], g = col[1], r = col[2];
                    result.at<uchar>(i, j) = (!isWhite(b, g, r) && r > minR && b < maxB && g < maxG && r > b + g) ? 255
                                                                                                                  : 0;
                }
        }
        cv::Mat blurred;
    ...
    std::vector<cv::RotatedRect> findLights(const cv::Mat &color, const cv::Mat &binary) {
        ...
        std::sort(lights.begin(), lights.end(),
                  [](const auto &lhs, const auto &rhs) { return lhs.center.x < rhs.center.x; });
        return lights;
    }
    ...
    std::vector<PairedLight>
    matchLights([[maybe_unused]] const cv::Mat &src, const std::vector<cv::RotatedRect> &lights) {
        ACTOR_EXCEPTION_PROBE();
...
public:
    ArmorDetector(caf::actor_config &base, const HubConfig &config) : HubHelper{base, config},
                                                                      mKey{generateKey(this)} {}

    caf::behavior make_behavior() override {
        return {[this](start_atom) {
            ACTOR_PROTOCOL_CHECK(start_atom);
            GlobalSettings::get().selfColor = Color::Blue;
        },
