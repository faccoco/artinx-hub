                 ...
                 [&](armor_detect_available_atom, Identifier key) {
                     ...
                     for(const auto& armor : res.armors) {
                         ...
                         for(int i = 0; i < 4; i++) {
                             cv::circle(
                                 showImg,
                                 cv::Point(static_cast<int>(armor.light4Point[i].x), static_cast<int>(armor.light4Point[i].y)), 2,
                         ...
                         // 绘制目标颜色与类别
                         std::string id(fmt::format("{} {}", magic_enum::enum_name(armor.robotType), armor.ratio > 2.8));
