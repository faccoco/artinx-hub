...
bool inspect(Inspector& f, Ros& x) {
    return f.object(x).fields(
        f.field("ip", x.ip).fallback("127.0.0.1"),
        f.field("imageTopic", x.imageTopic).fallback("/image"),
        f.field("cameraInfoTopic", x.cameraInfoTopic).fallback("/camera_info"));
}
//
class RosCamera final
    : public HubHelper<caf::event_based_actor, CameraBaseSettings, image_frame_atom> {
    Identifier mKey{};
...
public:
    RosCamera(caf::actor_config& base, const HubConfig& config, std::string name)
        ...
        //     "/image", rclcpp::SensorDataQoS(), std::bind(&RosCameraDriver::publisRosImg, this, std::placeholders::_1));
        //
    ...
    Transform<FrameOfRef::Robot, FrameOfRef::Camera, true> clcTfRobot2Camera(const Pose& gunPose) {
        const double yaw = gunPose.yaw;
        const double pitch = mConfig.isAtGun ? gunPose.pitch : 0;
        const double roll = gunPose.roll;
        return glm::rotate(
