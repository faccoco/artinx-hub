...
class InfantrySerialPort final : public HubHelper<caf::event_based_actor, InfantrySerialPortSettings, update_head_atom,
    ...
    void infantryRecvCB(const InfantryRecvPacket& fdb) {
        ...
        GlobalSettings::get().pitchFromserial = mPitch;
        GlobalSettings::get().yawFromserial = mYaw;
        HubLogger::watch("pitchFromserial", GlobalSettings::get().pitchFromserial);
        HubLogger::watch("yawFromserial", GlobalSettings::get().yawFromserial);
